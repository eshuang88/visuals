...
(function (global) {
  ...
  function start(opts) {
    ...
    // Performance start screen: a plain near-black click-to-start surface with
    // NO text of any kind. The operator knows to click (fullscreen + mic need a
    // user gesture); the audience must never see anything that isn't the show.
    overlay.innerHTML = '';
    overlay.style.cssText = 'background:#05070a;';
    ...
    // a still image bound to u_tex (cover slides). Uploaded once when it loads.
    function makeImage(src) {
      ...
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
      var rec = { image: img, tex: tex, ready: false, uploaded: false, isImage: true };
      img.addEventListener('load', function () { rec.ready = true; });
      img.src = (typeof src === 'string') ? src : (src && src.src);
      return rec;
    }

    // a minimal black program — the fallback for any scene that won't compile,
    // so one bad track never freezes the live show (it just shows black).
    var blackP = null;
    function blackProgram() {
      if (blackP) return blackP;
      ...
      gl.linkProgram(prog);
      blackP = { prog: prog, loc: gl.getAttribLocation(prog, 'a_pos'),
                 U: { res: null, time: null, bass: null, flow: null, beat: null, tex: null, texRes: null },
                 vid: null };
      return blackP;
    }

    // one program per track (compiled lazily on first show, then cached). A
    // compile/link failure is caught and swapped for the black program (logged),
    // so the player keeps running through a broken scene.
    var programs = new Array(tracks.length);
    function programFor(i) {
      if (programs[i]) return programs[i];
      var fs;
      try {
        fs = compile(gl.FRAGMENT_SHADER, PRELUDE + '\n' + tracks[i].scene);
        var prog = gl.createProgram();
        gl.attachShader(prog, vs);
        ...
        console.error('scene ' + i + ' "' + (tracks[i].title || '') + '" failed to compile:\n' + e.message);
        programs[i] = blackProgram();
        return programs[i];
      }
      ...
      var P = {
        ...
        vid: tracks[i].image ? makeImage(tracks[i].image)
           : tracks[i].video ? makeVideo(tracks[i].video) : null
      };
      programs[i] = P;
      return P;
    }
    ...
    function bindFootage(P) {
      gl.activeTexture(gl.TEXTURE0);
      if (P.vid && P.vid.isImage) {
        var im = P.vid.image;
        gl.bindTexture(gl.TEXTURE_2D, P.vid.tex);
        if (P.vid.ready && !P.vid.uploaded && im.naturalWidth) {
          try { gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, im); P.vid.uploaded = true; } catch (e) {}
        }
        if (P.U.texRes) gl.uniform2f(P.U.texRes, P.vid.uploaded ? im.naturalWidth : 0, P.vid.uploaded ? im.naturalHeight : 0);
      } else if (P.vid) {
        var v = P.vid.video;
        gl.bindTexture(gl.TEXTURE_2D, P.vid.tex);
    ...
    function syncVideoPlayback() {
      for (var i = 0; i < programs.length; i++) {
        var P = programs[i];
        if (!P || !P.vid || P.vid.isImage) continue;
    ...
    var micLevel = 0, silence = 0, fbWeight = 1, micAvg = 0;
    ...
    // live-sound level from the mic: a low band drives the slow undulation (the
    // house "swell"), plus a wider band so ANY room sound — voices, mids,
    // transients — also registers. micLevel's 0.25 smoothing (below) low-passes
    // this into a slow envelope, so motion stays emotion-sync, never metronomic.
    function readBass() {
      if (!analyser) return 0;
      analyser.getByteFrequencyData(freq);
      var lo = 0, wide = 0, i;
      for (i = 1; i <= 8;  i++) lo   += freq[i];   // ~40–350 Hz  (the swell)
      for (i = 1; i <= 48; i++) wide += freq[i];   // up to ~2 kHz (any sound)
      lo /= 8 * 255; wide /= 48 * 255;
      return Math.min(1, lo * 1.15 + wide * 0.85);
    }
    ...
    function togglePause() { playing = !playing; syncVideoPlayback(); }
    ...
    function frame(now) {
      ...
      if (playing) {
        var raw = readBass();
        // adaptive gain: normalise against a slow estimate of the room's own
        // level so the visuals get full dynamics whether the room is quiet or
        // loud. micAvg adapts over ~seconds (not per-beat), so beats read as
        // dynamics above it and it never pumps; a floor keeps true silence
        // silent (-> the BPM fallback below).
        micAvg += (raw - micAvg) * 0.02;
        var norm = Math.min(1, raw / Math.max(micAvg * 1.8, 0.12));
        micLevel += (norm - micLevel) * 0.25;
