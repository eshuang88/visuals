...
(function (global) {
  ...
  var PRELUDE = [
    ...
    'uniform sampler2D u_tex;   // footage frame (0 when a track has no video)',
    'uniform vec2  u_texRes;    // footage pixel size (0 when unused)',
    '',
    '// cover-fit a 0..1 stage uv onto the footage (CSS "cover": fill 4:3, crop the',
    '// overflow). Only meaningful for footage tracks that pass opts.video.',
    'vec2 texCoverUV(vec2 uv01){',
    '  float ta = u_texRes.x / max(u_texRes.y, 1.0);   // footage aspect',
    '  float sa = u_res.x    / max(u_res.y, 1.0);       // stage aspect (4:3)',
    '  vec2 c = uv01 - 0.5;',
    '  if (ta > sa) c.x *= sa / ta; else c.y *= ta / sa;',
    '  return c + 0.5;',
    '}',
    '// il_coverUV / il_frame — an alternate footage-sampling convention some',
    '// tracks were authored against (cave-human). Additive: tracks that use',
    '// texCoverUV/u_tex directly are unaffected.',
    'vec2 il_coverUV(vec2 uv){',
    ...
    '  return (uv - 0.5) * s + 0.5;',
    '}',
    'vec3 il_frame(vec2 uv){ return texture2D(u_tex, il_coverUV(uv)).rgb; }',
    '',
  ...
  function start(opts) {
    ...
    // ---------------- optional footage texture (opts.video) ----------------
    // Additive + backward compatible: generative tracks pass no video, so a 1x1
    // black texture stays bound on unit 0 and u_texRes = 0 — scenes that never
    // sample u_tex are unaffected. Footage tracks pass opts.video (a URL, or an
    // array of {src,type} sources) and sample u_tex / texCoverUV() in the scene.
    ...
    if (opts.video) {
      ...
      var sources = Array.isArray(opts.video) ? opts.video
                  : [{ src: opts.video }];
      ...
      // keep it in the DOM (hidden) — detached media can be throttled / not decoded
      video.style.cssText = 'position:absolute;width:1px;height:1px;opacity:0;pointer-events:none;left:-10px;top:-10px;';
      ...
      tryPlay(); // muted autoplay is allowed; begin() retries after the gesture
    }
    ...
    function begin() {
      ...
      resize();
      if (video) { var pp = video.play(); if (pp && pp['catch']) pp['catch'](function () {}); }
