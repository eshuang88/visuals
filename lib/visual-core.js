 ...
 *
 * Footage tracks (optional): pass a seamless-loop clip and the engine keeps its
 * current frame in the u_tex sampler for the scene to treat (grade/chroma/grain):
 *
 *   Ambient.start({ title, bpm, video: 'assets/loop.mp4' });   // or a data: URI
 *
 * Sample it with il_frame(uv) (cover-fit into 4:3) or u_tex/u_texres directly.
 * Generative tracks pass no video and u_tex stays unused — fully backward compatible.
 *
...
(function (global) {
  ...
  var PRELUDE = [
    ...
    '',
    '// ---- footage (optional): bound only when a track passes {video} to start() ----',
    'uniform sampler2D u_tex;    // current video frame (black if no footage track)',
    'uniform vec2      u_texres; // native pixel size of that frame (0 if none)',
    '',
    ...
    '}',
    '',
    '// ---- footage helpers: cover-fit the video into the 4:3 frame (no crop distortion) ----',
    'vec2 il_coverUV(vec2 uv){                 // uv in 0..1 over the frame -> uv into u_tex',
    '  if (u_texres.x < 1.0 || u_texres.y < 1.0) return uv;   // no footage bound',
    '  float fa = u_res.x / u_res.y, ta = u_texres.x / u_texres.y;',
    '  vec2 s = (fa > ta) ? vec2(1.0, ta/fa) : vec2(fa/ta, 1.0);',
    '  return (uv - 0.5) * s + 0.5;',
    '}',
    'vec3 il_frame(vec2 uv){ return texture2D(u_tex, il_coverUV(uv)).rgb; }  // straight cover-fit sample',
    '',
  ...
  function start(opts) {
    ...
    // ---------------- optional footage: loop a video into u_tex ----------------
    // A footage track passes {video:<url|dataURI>}. The clip should already be a
    // seamless 4:3 loop (see build-video-datauri.js); the scene shader then treats
    // it as raw material (grade, chroma, grain) via il_frame()/u_tex. Backward
    // compatible: generative tracks pass no video and u_tex stays unused.
    var video = null, videoTex = null, videoReady = false;
    if (opts.video) {
      ...
      video.setAttribute('muted', ''); video.preload = 'auto';
      // kept in the DOM but invisible — some browsers pause off-DOM / display:none video
      ...
      video.addEventListener('loadeddata', function () { videoReady = true; tryPlay(); });
      tryPlay(); // muted autoplay is allowed in most browsers; begin() retries on the gesture
    }
    ...
    function uploadVideo() {
      ...
      if (U.texres) gl.uniform2f(U.texres, video.videoWidth || 0, video.videoHeight || 0);
    }
